...
const Quiz: React.FC<QuizProps> = ({ appData, onUpdateWord }) => {
    ...
    const ResultItem = ({ result }: { result: QuizResult }) => (
      <div className="flex items-start justify-between p-3 rounded-lg bg-slate-100 dark:bg-slate-700/50">
        ...
        <button
          ...
          className={`p-1 rounded-full transition-colors cursor-pointer ${result.word.isStarred ? 'text-yellow-400 hover:text-yellow-300' : 'text-slate-400 hover:text-yellow-400'}`}
    ...
    return (
        <div className="bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-xl shadow-lg">
            ...
            <div className="flex justify-center gap-4 mb-6">
              <button onClick={() => startQuiz(lastQuizFolder)} className="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors font-semibold cursor-pointer">다시하기</button>
              <button onClick={resetQuiz} className="px-6 py-2 bg-slate-600 text-white rounded-lg hover:bg-slate-700 transition-colors font-semibold cursor-pointer">폴더 선택으로</button>
            </div>
        ...
        <div className="bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-xl shadow-lg text-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                {appData.folders.map(folder => (
                    <button key={folder.id} onClick={() => startQuiz(folder.id)} className="flex items-center justify-center gap-2 p-4 bg-slate-600 text-white rounded-lg hover:bg-slate-700 transition-colors font-semibold cursor-pointer">
  ...
  return (
    <div className="bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-xl shadow-lg text-center relative">
      {currentWord ? (
        <>
          <button onClick={resetQuiz} className="absolute top-4 right-4 text-sm text-slate-400 hover:text-slate-200 cursor-pointer">폴더 선택으로</button>
              ...
              <div className="mt-4">
                ...
                {wordIndex < quizWords.length - 1 ? (
                   <button
                    ...
                    className="w-full mt-4 bg-slate-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-slate-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900 focus-visible:ring-slate-500 transition-transform transform active:scale-95 shadow-md cursor-pointer"
                  >
                    다음 단어
                  </button>
                ) : (
                   <button
                    ...
                    className="w-full mt-4 bg-indigo-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-indigo-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900 focus-visible:ring-indigo-500 transition-transform transform active:scale-95 shadow-md cursor-pointer"
                  >
                    결과 보기
                  </button>
                )}
