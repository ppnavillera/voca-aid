...
const WordList: React.FC<WordListProps> = ({ appData, onUpdateWord }) => {
  ...
  const FilterButton: React.FC<{
    ...
    <button
      ...
      className={`px-3 py-1.5 text-sm font-semibold rounded-full transition-colors flex items-center gap-1.5 cursor-pointer ${
        selectedFolderId === id
                ...
                <div className="flex items-center justify-between">
                  <div>
                   ...
                   <button
                    ...
                    className={`p-1 rounded-full transition-colors cursor-pointer ${word.isStarred ? 'text-yellow-400 hover:text-yellow-300' : 'text-slate-400 hover:text-yellow-400'}`}
